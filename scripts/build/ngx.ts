import { CompilerHost, CompilerOptions, createCompilerHost, createProgram } from '@angular/compiler-cli';
...
export function getProgram(rootNames: string[] = createSourceFiles()) {
  ...
  options.rootDir = resolve(ROOT, options.rootDir || 'src');
  options.outDir = resolve(ROOT, options.outDir || 'dist');
